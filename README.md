...
- **firmware_sdcard/firmware_gif_sd:** MicroSD на отдельной SPI-шине
  ...
  CS=GPIO38, SCK=GPIO39, MOSI=GPIO40, MISO=GPIO41. Опциональная сенсорная
  кнопка (TTP223) на GPIO15 — рефреш/следующий GIF смотря по прошивке.
  **VCC модуля — 5V, не 3.3V** (свой AMS1117 с просадкой ~1В, иначе SD не
  проходит инициализацию). На типовых ESP32-S3 devkit-платах может
  потребоваться запаять перемычку, чтобы 5V-пин реально шёл от USB VBUS
- **firmware_gif_sd:** дополнительно нужен `secrets.h` (тот же паттерн, что у
  radar/clouds/cloudradar/map, но без домашних координат) — Wi-Fi тут не
  обязателен для показа гифок, только для веб-морды загрузки/удаления файлов
  на порту 80. BTN1 короткое нажатие — пауза, долгое (>600мс) — предыдущий GIF
