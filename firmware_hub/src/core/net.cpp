...
bool fetchToBuffer(const String& url, uint8_t* buf, size_t bufSize, size_t& outLen,
    ...
    if (client) http.begin(*client, url);
    else        http.begin(url);
    int code = http.GET();
...
// Корни подтверждены напрямую (openssl s_client/curl -v) в firmware_map
const char* ROOT_CA_AMAZON = R"EOF(
-----BEGIN CERTIFICATE-----
