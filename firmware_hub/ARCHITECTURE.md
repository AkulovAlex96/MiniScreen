...
└── src/
    ...
    └── screens/
        ...
        └── crypto_ticker.cpp # курс криптовалюты (Binance), 4 слота crypto1..crypto4
```
...
public:
    ...
    virtual bool tick(uint32_t nowMs) = 0;   // кадр в spr; true = обновлён, ядро запушит
...
- **Экран НЕ пушит спрайт сам**: `tick()` возвращает true, если кадр обновлён —
  ядро вкомпонует оверлей (название экрана после переключения) и сделает один
  `pushSprite`, без моргания. Редко-рисующие экраны (rain_radar, city_map,
  sd_info) возвращают false, пока кадр не менялся. Исключение — статус перед
  блокирующим фетчем: `statusScreen(...)` + явный `spr.pushSprite(0,0)`.
- **Сеть экран дергает сам внутри `tick()`** по своим таймерам, через helpers из
  `net.h` (общий `secureClient`, `fetchToBuffer`). Блокирующий фетч на 1-2 с
  внутри tick допустим — как в прототипах.
- **Состояние между заходами** можно держать в static'ах файла, но большие буферы
  (PSRAM-спрайты) — только между onEnter/onExit. Небольшие кэши СЖАТОГО контента
  (PNG карты/тайла, 48КБ PSRAM) живут постоянно: при возврате на экран — повторный
  декод вместо перекачки, сеть не трогается (radar, rain_radar, city_map).
...
| кэши сжатых PNG (radar/rain_radar/city_map, по 48 КБ) | PSRAM (ps_malloc), постоянные | 144 КБ |
...
PNG/GIF-декодеры — **общие** (экран активен один за раз); три копии, как было
бы при слепом объединении прототипов, не влезли бы. Кэши сжатых PNG наоборот
**у каждого экрана свои и постоянные** — 48 КБ PSRAM стоят того, чтобы при
каждом переключении экранов не ходить в сеть за картой/тайлом заново.
...
| — | crypto1..crypto4 (crypto_ticker.cpp) | ✅ новый экран, не миграция: курс криптовалюты с Binance, 4 независимых слота |
